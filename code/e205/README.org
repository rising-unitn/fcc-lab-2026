* Exercise 205 - Make two containers talk to each other
  - Description :: Create a custom bridged network. Create two different containers (a client and a server) and attach them to the same network. Finally use Docker's internal DNS to communicate from one to the other.
