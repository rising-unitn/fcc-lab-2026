* Exercise 207 - Run a custom Service with Docker
  - Description :: Using the =nginx= container image at https://hub.docker.com/_/nginx/, set up a simple webserver. Replace the nginx default page with custom content. Expose its internal port on port =8080= of the Docker host and connect to it. Finally change the custom content exposed by the webserver.
...
* Solutions and Instructions
    ...
    8) Check the web-server page now
       ...
       #+END_SRC
    9) Destroy and restart the container
       /Let us now assume that for some reason the container must be restarted by an admin, the server hosting the container goes down, or the container restarts due to a fatal application error. Of course we will simulate this bad situation (that honestly in production may appear very frequently) using the following commands:/
       #+BEGIN_SRC sh
